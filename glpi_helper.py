"""
Classe auxiliar para interação com a API do GLPI.

Esta classe fornece uma interface orientada a objetos para comunicação
com o GLPI, incluindo gerenciamento de sessões e operações CRUD em tickets.
"""
...
class GLPIHelper:
    """
    Classe para interagir com a API do GLPI.
    """
    ...
    def __init__(self, base_url: str, user_token: str, app_token: str):
        ...
        Args:
            ...
            user_token: Token do usuário para autenticação
    ...
    def init_session(self) -> bool:
        ...
        Returns:
            True se a sessão foi iniciada com sucesso, False caso contrário
        """
    ...
    def kill_session(self) -> bool:
        ...
        try:
            ...
            if success:
                self.session_token = None
                logger.info("Sessão GLPI finalizada com sucesso")
            else:
                logger.error(f"Falha ao finalizar sessão GLPI: {response.status_code} - {response.text}")
            return success
        except Exception as e:
    ...
    def get_ticket_details(self, ticket_id: int) -> Optional[Dict[str, Any]]:
        ...
        """
        if not self.session_token:
            logger.error("Nenhuma sessão ativa. Inicie uma sessão primeiro.")
            return None
    ...
    def update_ticket(self, ticket_id: int, ticket_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Atualiza um ticket existente no GLPI.
        
        Args:
            ticket_id: ID do ticket a ser atualizado
            ticket_data: Dicionário com os dados a serem atualizados
            
        Returns:
            Dicionário com informações do ticket atualizado ou None em caso de erro
        """
        if not self.session_token:
            logger.error("Nenhuma sessão ativa. Inicie uma sessão primeiro.")
            return None
        ...
        payload = {'input': update_data}
        
        try:
            response = requests.put(url, headers=headers, json=payload, verify=False)
            if response.status_code in [200, 201]:
                result = response.json()
                logger.info(f"Ticket {ticket_id} atualizado com sucesso!")
                return result
            else:
                logger.error(f"Falha ao atualizar ticket {ticket_id}: {response.status_code} - {response.text}")
                return None
        except Exception as e:
    ...
    def get_ticket_solution(self, ticket_id: int) -> List[Dict[str, Any]]:
        """
        ...
        url = f"{self.base_url}/Ticket/{ticket_id}/ITILSolution"
        headers = {
            'Content-Type': 'application/json',
            'Session-Token': self.session_token,
            'App-Token': self.app_token
    ...
    def add_followup_to_ticket(self, ticket_id: int, content: str, is_private: int = 0) -> Optional[Dict[str, Any]]:
        """
    ...
    def search_ticket_by_id(self, ticket_id: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def search_ticket_by_external_id(self, external_id: str) -> Optional[int]:
        """
    ...
    def get_ticket_logs(self, ticket_id: int) -> List[Dict[str, Any]]:
        ...
        url = f"{self.base_url}/Ticket/{ticket_id}/Log"
        headers = {
            'Content-Type': 'application/json',
            'Session-Token': self.session_token,
            'App-Token': self.app_token
    ...
    def get_all_tickets_with_history(
        ...
        # Busca os tickets
        search_result = self.search_all_tickets(
        ...
        for ticket_dict in tickets_data:
            ...
            if include_solutions:
                solutions = self.get_ticket_solution(ticket_id)
    ...
    def create_ticket_in_glpi(
        ...
        """
        Cria um novo ticket no GLPI, carregando o histórico do Supabase da mesma forma 
        que o endpoint create_ticket do webhook.
        
        Args:
            user_chat_id: ID do chat para carregar título e histórico de mensagens
            title: Título do ticket (opcional, será carregado do Supabase se não fornecido)
            ...
            conversation_history: Lista com histórico da conversa (opcional)
                ...
                Se não fornecido, será carregado do Supabase
        ...
        ticket_data = {
            ...
            'externalid': user_chat_id  # Usa user_chat_id como externalid
        }
